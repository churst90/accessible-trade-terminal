...
public static class DiagnosticCommand
{
        ...
        OrderSide side = OrderSide.Buy)
    {
        ...
        Console.WriteLine();
        Console.WriteLine($"Testing {markers.Count} marker signals as single-leaf {(side == OrderSide.Buy ? "long" : "short")} strategies, 4 modes each:");
        ...
        foreach (var sig in markers)
        {
            ...
            var spec = MakeSingleLeafSpec(sig.Id, sig.DisplayLabel, side);
    ...
    private static StrategySpec MakeSingleLeafSpec(string descriptorId, string label, OrderSide side = OrderSide.Buy)
    {
        ...
        var sideTag = side == OrderSide.Buy ? "long" : "short";
        return new StrategySpec(
            Id: $"diag.{sideTag}.{descriptorId}",
            Name: $"DIAG ({sideTag}): {label}",
