...
namespace AccessibleTrader.Core.Services
{
    ...
    {
        private readonly ConcurrentDictionary<string, PaperTradingProvider> _accounts =
            new(StringComparer.Ordinal);

        /// <summary>
        /// This user's account, creating it once on first use. <paramref name="create"/> is only
        /// invoked for a key that has none.
        /// </summary>
        public PaperTradingProvider ForUser(string userKey, Func<PaperTradingProvider> create)
        {
            ...
            return _accounts.GetOrAdd(userKey, _ =>
            {
                var account = create();
        ...
        public void Dispose()
        {
            ...
            {
                kv.Value.SharedOwnership = false;
                kv.Value.DisposeAccount();
            }
            _accounts.Clear();
        }
    }
