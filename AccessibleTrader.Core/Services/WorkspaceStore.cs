    ...
    public class WorkspaceStore : IWorkspaceStore, IDisposable
    {
        ...
        public void Dispatch(WorkspaceAction action)
        {
            ...
            lock (_lock)
            {
                ...
                //
                // IsLogScale is deliberately NOT here: it changes how the range is MAPPED to
                // the pane (ChartMath.MapY / NormalizedPosition read the flag at draw and at
                // sound time), not what the range IS. Anything that changes what the range is
                // must be here, or it is silent until the next tick happens to recompute —
                // which on a paused or closed market is never. Hiding a component changes
                // ActiveSeries, so the fit follows visibility.
