...
| | holds | ships to |
|---|---|---|
| **AccessibleTrader.StrategyLab** (`Catalogue/`) | the 30 strategy specs and their provenance | nobody — it is a research tool |
...
The current split of the 30: **1** control-tested, **5** walk-forward, **9** in-sample-only, **7**
untested, **2** fragile, **6** falsified — and the one control-tested entry is the plain trend
baseline, whose verdict is that the family works while its own fitted parameters carry no
information. That is the honest state of five months of strategy work, and it is the reason the
terminal stopped shipping these as a starting library.
...
`CIPHER_SR` leaf in its entry stack is optimistic by an unmeasured amount. Seven specs are affected.

### Libraries that predate the split

An existing install keeps the thirty specs it was already seeded with, and those rows have no
