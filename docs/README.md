...
- **Tests:** `AccessibleTrader.Tests` — Unit and integration diagnostics (8088 tests, all passing); `AccessibleTrader.BrowserTests` — a Playwright harness that drives a real Chromium against a real Kestrel running the WebHost, asserting per-modal `document.activeElement` against a declared target, dialog accessible names, Tab traps, Escape and focus return, an accessible-name sweep over every reachable control, and what the terminal SAYS along with its interrupt flag. Plus five zero-dependency JS suites — the gesture engine (`node tools/jstests/gesture-tests.mjs`, 15 tests), the keydown trap (`node tools/jstests/keyboard-tests.mjs`, 75 tests), which asserts on `preventDefault`, something no C# test can observe, and the ARIA tree keyboard model behind the Object Tree (`node tools/jstests/tree-tests.mjs`, 25 tests), and the WebHost's circuit start and reconnection back-off (`node tools/jstests/boot-tests.mjs`, 8 tests), and what is announced while it is down (`node tools/jstests/reconnect-tests.mjs`, 5 tests). All of them run in CI.
