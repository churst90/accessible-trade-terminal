...
Suite 2109 → 2816, plus 15 JS gesture tests.
...
- **Plugins:** `Plugins/` — 31 exchange, data, and analytics provider plugins (14 trading + 17 analytics).
...
- **Tests:** `AccessibleTrader.Tests` — Unit and integration diagnostics (2816 tests, all passing), plus a zero-dependency JS gesture-engine suite (`node tools/jstests/gesture-tests.mjs`, 12 tests). Both run in CI.
