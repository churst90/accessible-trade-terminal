...
**Since 2.3.0 tagged, main has taken 107 commits and none of them added a feature** — that is the
whole of **2.4.0**, tagged 2026-08-28. The period went to finding out what was actually wrong and
then fixing it, in that order. Suite **5,897** in both configurations (`--list-tests` reports 5892,
which is the number this file must match and `doc-drift.yml` checks — the two differ because
`--list-tests` names a `[Theory]` once and a run expands its cases). CI green on all three
workflows that run on a push. **0 CRITICAL and 0 HIGH items open.**
