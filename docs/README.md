...
test that now enforces it. Suite at 2334.
...
- **Tests:** `AccessibleTrader.Tests` — Unit and integration diagnostics (2334 tests, all passing), plus a zero-dependency JS gesture-engine suite (`node tools/jstests/gesture-tests.mjs`, 12 tests). Both run in CI.
